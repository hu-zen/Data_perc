...
class AmbilDataKasar:
    def __init__(self):
        rospy.init_node('node_ambil_data_kasar')
        ...
        # Parameter (JANGAN UBAH AGAR SAMA DENGAN DATA HALUS)
        self.kecepatan_uji = 0.5
        self.durasi_jalan = 5.0
        self.durasi_diam = 2.0

        rospy.sleep(1) # Tunggu koneksi
    ...
    def run(self):
        ...
        print("\n[STATUS] Mode Robot: KASAR (Direct Control)")
        rospy.sleep(1)
        ...
        self.is_recording = False
        f.close()
        self.pub_vel.publish(msg) # Pastikan diam total
        print(f"[SELESAI] Data tersimpan di: {filename}")
        print("Silakan kembalikan robot ke titik START secara manual.")
